...
class WordPressMLMAccessControl 
{
	...
	function show_column( $name ) 
	{
		...
		switch ( $name ) {
			case 'mlm':
				if ( get_post_meta( $post->ID, '_mlm_is_members_only', true ) ) {
					echo '<img src="' . plugin_dir_url( __FILE__ ) . 'images/lock.png" alt="Members Only" title="Members Only" />';
				} else if ( get_post_meta( $post->ID, '_mlm_is_nonmembers_only', true ) ) {
					echo '<img src="' . plugin_dir_url( __FILE__ ) . 'images/unlock.png" alt="Non-members Only" title="Non-members Only" />';
				}
	...
	function check_for_members_only()
	{
		...
		if ( get_post_meta( $post->ID, '_mlm_is_members_only', true ) && ! WordPressMLMAccessControl::check_conditions( $post->ID ) ) {
			if ( is_singular() ) {
				$redirect_to = get_post_meta( $post->ID, '_mlm_members_redirect_to', true );
	...
	function check_for_nonmembers_only()
	{
		...
		if ( get_post_meta( $post->ID, '_mlm_is_nonmembers_only', true ) && ! WordPressMLMAccessControl::check_conditions( $post->ID ) ) {
			$redirect_to = get_post_meta( $post->ID, '_mlm_nonmembers_redirect_to', true );
	...
	function remove_excerpt( $excerpt )
	{
		...
		if ( get_post_meta( $post->ID, '_mlm_is_members_only', true ) && ! WordPressMLMAccessControl::check_conditions( $post->ID ) ) {
			$show_excerpt = get_post_meta( $post->ID, '_mlm_show_excerpt_in_search', true );
	...
	function remove_restricted_posts( $posts, $query = false )
	{
		...
		if ( ! is_singular() && ! is_admin() ) {
			...
			foreach ( $posts as $post ) {
				...
				if ( get_post_meta( $post->ID, '_mlm_is_members_only', true ) && ! WordPressMLMAccessControl::check_conditions( $post->ID ) ) {
					if ( get_post_meta( $post->ID, '_mlm_show_in_search', true ) != 1 ) {
						continue;
					}
				} 
