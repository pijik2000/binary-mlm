...
Epoch.prototype.createMainHeading = function () //PRIVATE: Creates the primary calendar heading, with months & years
{
	...
	monthUp.setAttribute('class',this.monthup_class);
	monthUp.setAttribute('className',this.monthup_class);
	monthUp.setAttribute('title',this.monthup_title);
	monthDn.setAttribute('type','button');
	monthDn.setAttribute('value','<');
	monthDn.setAttribute('class',this.monthdn_class);
	monthDn.setAttribute('className',this.monthdn_class);
...
Epoch.prototype.createFooter = function () //PRIVATE: creates the footer of the calendar - goes under the calendar cells
{
	...
	clearSelected.setAttribute('class',this.clearbtn_class);
	clearSelected.setAttribute('className',this.clearbtn_class);
	clearSelected.owner = this;
	clearSelected.onclick = function() { this.owner.resetSelections(false);};
	container.appendChild(clearSelected);
	//clearSelected.setClass(clearbtn,'clearbtn');className
	return container;
};
//-----------------------------------------------------------------------------
...
function changefocus(name,period_lastnight)
{
	//alert(name);
	if(name == "dp_cal1"){
		var split_date = document.getElementById('search_arrival').value.split("/");
		dp_cal2.goToMonth(split_date[2],split_date[1]-1);
	}
	if(name == "popup1"){
		var split_date = document.getElementById('search_arrival1').value.split("/");
		popup2.goToMonth(split_date[2],split_date[1]-1);
	}
	if(name == "splofferpopup1"){
		var split_date = document.getElementById('offer_firstnight').value.split("/");
		splofferpopup2.goToMonth(split_date[2],split_date[1]-1);
	}
	if(name == "newfunction")
	{
		var split_date = period_lastnight.split("/");
		popup1.goToMonth(split_date[2],split_date[1]-1);
		popup2.goToMonth(split_date[2],split_date[1]-1);		
		//popup1.selectedDates = new Array();
		popup1.resetSelections();
		popup2.resetSelections();
		/*for(i=0;i<popup1.cells.length;i++) {
			popup1.cells[i].cellClass = '';
		}*/
	}
/*	if(name == "splofferpopup1"){
		var xmlhttp = false;
			...
			//Calling 				
				...
				var url = "offer-process.php?period_firstnight="+period_firstnight_final;	
				//alert(url);
				//return false;
				...
				xmlhttp.onreadystatechange = function() {
					if(xmlhttp.readyState == 4 && xmlhttp.status == 200) {
						...
						//alert(response);
						//return false;
 						document.getElementById("offer_lastnight").value = trim(response);						
 					}
				}
				xmlhttp.send(null); 
